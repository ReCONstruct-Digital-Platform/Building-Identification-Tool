...
def redeploy_server(request):
    ...
    for valid_ip in whitelist:
        ...
        return HttpResponse(
            status=403,
            reason=f"POST to /redeploy_server came from unauthorized IP: {client_ip_address}",
        )
    ...
    if event != "push":
        return HttpResponse(
            status=204, reason=f"Webhook event was not 'push' but {event}"
        )
    ...
    stashed = False
    if repo.is_dirty():
        log.warning(f"Repository is dirty. Stashing changes")
        repo.git.stash("save")
        stashed = True
    ...
    try:
        ...
        return HttpResponse(
            status=500,
            reason=f"Caught an exception while pulling. See server logs for details.",
        )

    if stashed:
        return HttpResponse(
            "Updated deployed version to new main branch. Some changes were stashed!"
        )
