    ...
    def handle(self, *args, **options):
        ...
        try:
            if not skip_import:
                ...
                cmds = [
                    f"shp2pgsql -c -D {'-a' if i > 0 else ''} -s 3857:4326 {lots_shp_folder}/{filename} {LOTS_TABLE_TMP} | psql -q {DB_CONN_STR}" \
                        for i, filename in enumerate(filenames)
                ]
                for cmd in cmds:
                    subprocess.check_call(cmd, shell=True)
                    
                self.stdout.write(self.style.SUCCESS(f'Finished importing lots to temporary table in {datetime.now() - t0}s'))
                ...
                self.stdout.write(self.style.SUCCESS(f'Finshed cleaning data and transfered to final lots table'))
...
def link_lots_to_evalunits(self, delete_data=False, test=False):
    ...
    for offset in range(0, num_lots, chunk_length):
        ...
        cursor.execute(f"SELECT gid, id_provinc FROM {LOTS_TABLE} ORDER BY gid DESC LIMIT {chunk_length} OFFSET {offset};")
