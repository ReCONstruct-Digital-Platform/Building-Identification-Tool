...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('-si', '--skip-import', 
                            action='store_true', 
                            default=False,
    ...
    def handle(self, *args, **options):
        ...
        try:
            if not skip_import:
                ...
                if not list(lots_shp_folder.glob('**/usage_predominant_s_2022.shp')) or download_data:
                    download_file(
                        "https://f005.backblazeb2.com/file/bit-data-public/quebec_lots_shapefiles.zip", 
                        lots_shp_folder, 
                        unzip=True
                    )
                    self.stdout.write(self.style.SUCCESS('Lot polygon shapefile downloaded successfully'))
                self.stdout.write("\nImporting the lots polygons... This should take around 10 minutes.\n")
                ...
                filenames = ["usage_predominant_s_2022.shp", "usage_predominant_s_2022_1.shp"]
                if test:
                ...
                cmds = [
                    f"shp2pgsql -D {'-I' if i == 0 else ''} {'-a' if i > 0 else ''} -s 3857:4326 {lots_shp_folder}/{filename} {LOTS_TABLE} | psql -q {DB_CONN_STR}" \
                        for i, filename in enumerate(filenames)
                ]
                for cmd in cmds:
                    subprocess.check_call(cmd, shell=True)
                    
                self.stdout.write(self.style.SUCCESS(f'Finished importing lots in {datetime.now() - t0}s'))

                # Delete street lots
                count = delete_lots_without_cubf()
                self.stdout.write(self.style.SUCCESS(f'Deleted {count} lots that did not have a CUBF'))
            ...
            self.stdout.write(self.style.SUCCESS(f'Finished processing lots in {datetime.now() - t0}s'))
...
def delete_lots_without_cubf():
    ...
    cursor.execute(f"SELECT count(*) FROM {LOTS_TABLE} WHERE utilisatio IS null")
    count = cursor.fetchone()[0]

    cursor.execute(f"DELETE FROM {LOTS_TABLE} WHERE utilisatio IS null")
    conn.commit()
...
def link_lots_to_evalunits(self, delete_data=False, test=False):
    ...
    cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_id_provinc ON {LOTS_TABLE} (id_provinc);")
    ...
    for offset in range(0, num_lots, chunk_length):
        ...
        cursor.execute(f"SELECT gid, id_provinc, geom FROM {LOTS_TABLE} ORDER BY gid DESC LIMIT {chunk_length} OFFSET {offset};")
        lots = cursor.fetchall()

        for i, lot in enumerate(lots):
            gid, id_provinc, geom = lot

            # For lots without a single provincial ID, do a spatial intersection search
            if id_provinc == 'Multiple':
                cursor.execute(f"""
                    UPDATE {EVALUNIT_TABLE} AS e 
                    SET lot_geom = st_simplify(r.geom, 0.000005, true), lot_id = r.no_lot
                    FROM (
                        SELECT e.id as id, l.geom as geom, l.no_lot as no_lot 
                        FROM lots l JOIN evalunits e ON ST_Intersects(l.geom, e.point) 
                        WHERE l.gid = %s 
                    ) AS r WHERE r.id = e.id;""", (gid,))

            # For lots with a single provincial ID, simply retrieve the evaluation unit
            else:
                cursor.execute(f"""UPDATE {EVALUNIT_TABLE} 
                               SET lot_id = %s, 
                                   lot_geom = st_simplify(%s, 0.000005, true)
                               WHERE id = %s""", 
                            (gid, geom, id_provinc))
    ...
    if delete_data and not test:
        cursor.execute(f"DROP TABLE {LOTS_TABLE};")
        conn.commit()
        self.stdout.write(self.style.SUCCESS('\tDeleted the lots table successfully.'))
