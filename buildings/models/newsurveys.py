...
def order_schema_by_question_number(schema):
    ...
    return dict(sorted(schema.items(), key=lambda x: x[1]["pos"]))
    ...
    def __init__(self, survey: Survey, *args, **kwargs):
        ...
        for field, config in schema.items():
            ...
            if "options" in config:
                # question options are dict with val, label and pos
                options = sorted(config["options"], key=lambda x: x["pos"])
                self.fields[field].widget.choices = [
                    (opt["val"], _(opt["label"]["en"])) for opt in options
                ]
