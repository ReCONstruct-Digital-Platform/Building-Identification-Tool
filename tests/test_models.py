...
class EvalUnitTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.eval_unit = EvalUnit.objects.create(id='id1', lat=1.0, lng=1.5, muni='mtl', year=2005, address='123 a st', mat18='fsd', cubf=1000, associated={'hlm': ['hlm1']})
        self.eval_unit2 = EvalUnit.objects.create(id='id2', lat=1.0, lng=1.5, muni='mtl',year=2005,  address='4656 a st', mat18='fsdfsd', cubf=1000, associated={'hlm': ['hlm1']})
 

    def test_cubf_name(self):
        # Original CUBF
        self.assertEqual(self.eval_unit.cubf_name(), "Logement")

        # Non-mapped CUBF name
        self.eval_unit.cubf = 1234
        self.assertEqual(self.eval_unit.cubf_name(), "")

    # TODO: Worked before as we returned the least voted unit, but broken bc of unofficially prioritzing HLMs
