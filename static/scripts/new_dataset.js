...
function getAllMappedColumns() {
  ...
  if (document.getElementById('building_details_selection').classList.contains('hidden') === false) {
    // Street components
...
function updateUnmappedColumnsSection(allColumns) {
  ...
  if (unmappedColumns.length > 0) {
    ...
    unmappedColumns.forEach(column => {
      ...
      dataTypeSelect.className = 'mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md';
...
function updateAllDropdownConstraints() {
  ...
  // Add building details selects if they're visible
  if (document.getElementById('building_details_selection').classList.contains('hidden') === false) {
    allSelects.push(
  ...
  allSelects.forEach((select) => {
    if (select) { // Make sure the select exists
      ...
      countCsvRows(file)
        .then((result) => {
          ...
          reader.onload = function (e) {
            ...
            zipColumnSelect.innerHTML = '<option value="">-- Select Postal Code Column --</option>';
            stateColumnSelect.innerHTML = '<option value="">-- Select Province Column --</option>';
            
            if (latColumnSelect && lngColumnSelect) {
              latColumnSelect.innerHTML = '<option value="">-- Select Latitude Column --</option>';
              lngColumnSelect.innerHTML = '<option value="">-- Select Longitude Column --</option>';
            }
            ...
            columns.forEach((column) => {
              ...
              // Auto-select building details fields if they exist
              if (document.getElementById('has_building_details_yes')) {
                // Check for construction year
                if (lowerColumn.includes("year") ||
                    lowerColumn.includes("built") ||
                    lowerColumn.includes("construction")) {
                  document.getElementById('has_building_details_yes').checked = true;
                  buildingDetailsSelection.classList.remove('hidden');
                  constYearColumnSelect.value = column;
                }
                
                // Check for number of floors
                if (lowerColumn.includes("floor") ||
                    lowerColumn.includes("storey") ||
                    lowerColumn.includes("story")) {
                  document.getElementById('has_building_details_yes').checked = true;
                  buildingDetailsSelection.classList.remove('hidden');
                ...
                if ((lowerColumn.includes("street") && lowerColumn.includes("num")) ||
                    lowerColumn === "number") {
                  document.getElementById('has_building_details_yes').checked = true;
                  buildingDetailsSelection.classList.remove('hidden');
        ...
        if (addressColumnSelect && addressColumnSelect.options.length > 1) {
          ...
          if (streetNumColumnSelect) {
            Array.from(streetNumColumnSelect.options).forEach(option => {
              const lowerValue = option.value.toLowerCase();
              if ((lowerValue.includes("street") && lowerValue.includes("num")) ||
                  lowerValue === "number") {
          ...
          if (constYearColumnSelect) {
            Array.from(constYearColumnSelect.options).forEach(option => {
              ...
              if (lowerValue.includes("year") ||
                  lowerValue.includes("built") ||
                  lowerValue.includes("construction")) {
          ...
          if (numFloorsColumnSelect) {
            Array.from(numFloorsColumnSelect.options).forEach(option => {
              const lowerValue = option.value.toLowerCase();
              if (lowerValue.includes("floor") ||
                  lowerValue.includes("storey") ||
                  lowerValue.includes("story")) {
